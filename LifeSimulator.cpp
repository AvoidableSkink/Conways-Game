...
// You'll want to set these equal to the width/height of the console the program
// runs within.
...
void LifeSimulator::insertPattern(const Pattern &pattern, std::uint8_t startX,
  ...
  for (size_t y = 0; y < pattern.getSizeY(); y++) {
    for (size_t x = 0; x < pattern.getSizeX(); x++) {
      ...
      // if in bounds update state of cell to be same as corresponding pattern
      // cell
...
// update - Performs a single step update of the world, following the four rules
// specified in the wiki article. for each cell, check its neighbors states and
...
// if a cell doesnt have all 8 neighbors the missing ones are just counted as
// dead
